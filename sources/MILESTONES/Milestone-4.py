...
import Temporal_Schemes.Stability_Regions as sr

from Cauchy_Problem.Cauchy_Problem import Cauchy_Problem
from ODE_problems.Harmonic_Oscillator import F_oscillator 

from numpy import array, linspace, cos, sin , transpose
...
#x = linspace(-5, 5, num=100);r
#y = linspace(-5, 5, num=100);

#dt = array([0.01, 0.1, 1])

#stab_region = sr.Stability_Region(x, y, ts.Leap_Frog)

#CSF = plt.contourf(x, y, transpose(stab_region), levels = [0, 1],  colors=['#C0C0C0'] ) #levels = [0.5, 1, 1.5]
#CS = plt.contour(x, y, transpose(stab_region), levels = [0.5, 1, 1.5] ) 
#for t in range(3):
